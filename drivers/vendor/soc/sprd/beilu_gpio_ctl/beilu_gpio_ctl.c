...
//#define BEILU_GPIO_DEBUG
#ifdef BEILU_GPIO_DEBUG
#define BL_DEBUG(fmt, args...) do { \
	printk(KERN_ERR "[bl mcu] %s:"fmt, __func__, ##args);\
} while (0)
#else
...
static ssize_t mcu_irq_read(struct file *file,
	...
	*offset += ret;
	...
	BL_DEBUG("count:%d ret:%d, irq_flag:%s,offset:%lld\n", count, ret, irq_flag, *offset);
	BL_DEBUG("finished\n");
	return ret;
}
...
static int beilu_gpio_init() {
	...
	if (ret < 0) {
		BL_ERR("mcu en gpio request failed!\n");
		return ret;
	}
	ret = gpio_direction_output(mcu_en_gpio,0);
	if (ret < 0) {
		BL_ERR("mcu en gpio set dir failed!\n");
		return ret;
	}
	...
	if (ret < 0) {
		BL_ERR("mcu irq gpio request failed!\n");
		return ret;
	}
	ret = gpio_direction_output(mcu_irq_gpio,0);
	if (ret < 0) {
		BL_ERR("mcu irq gpio set dir failed!\n");
		return ret;
	}
...
static void create_beilu_proc_entry(void) {
	...
	if (mcu_irq_proc_entry == NULL) {
		BL_ERR("Failed to create mcu irq entry\n");
		return;
	}
	return;
}
