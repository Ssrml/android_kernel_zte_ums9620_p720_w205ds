...
int axs_psensor_init(struct axs_ts_data *ts_data)
{
    ...
    if (!ts_data->axs_proximity_input_dev) {
		AXS_ERROR("%s: axs_proximity_input_dev allocate failed\n", __func__);
		return -ENODEV;
	}
	AXS_INFO("%s: axs_proximity_input_dev allocate success\n", __func__);
    ...
    AXS_INFO("%s: axs_proximity_input_dev input set success\n", __func__);
	ret = input_register_device(ts_data->axs_proximity_input_dev);
    if (ret) {
	    AXS_ERROR("%s: input_register_device failed\n", __func__);
