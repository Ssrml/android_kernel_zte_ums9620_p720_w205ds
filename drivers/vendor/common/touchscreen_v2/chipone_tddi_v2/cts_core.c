...
int cts_charger_plugin(struct cts_device *cts_dev)
{
	...
	//ret = cts_send_command(cts_dev, CTS_CMD_CHARGER_ATTACHED);
	/* ret = cts_dev->ops->set_charger_state(cts_dev,1); */
    ret = cts_tcs_set_charger_plug(cts_dev, 1);
	if (ret) {
		cts_err("Send TP_STD_CMD_SYS_STS_CHARGER_PLUGIN_RW:1 failed %d", ret);
	}
	return ret;
}
...
int cts_charger_plugout(struct cts_device *cts_dev)
{
	...
	//ret = cts_send_command(cts_dev, CTS_CMD_CHARGER_DETACHED);
	/* ret = cts_dev->ops->set_charger_state(cts_dev,0); */
    ret = cts_tcs_set_charger_plug(cts_dev, 0);
	if (ret) {
		cts_err("Send TP_STD_CMD_SYS_STS_CHARGER_PLUGIN_RW:0 failed %d", ret);
	}
	return ret;
}
...
#ifdef CFG_CTS_HEADSET_DETECT
int cts_earphone_plugin(struct cts_device *cts_dev)
{
	int ret;

	cts_info("Earphone plugin");
	//ret = cts_send_command(cts_dev, CTS_CMD_CHARGER_ATTACHED);
	/* ret = cts_dev->ops->set_earphone_state(cts_dev,1); */
    ret = cts_tcs_set_earjack_plug(cts_dev, 1);
	if (ret) {
		cts_err("Send TP_STD_CMD_SYS_STS_EP_PLUGIN_RW:1 failed %d", ret);
	}
	return ret;
}
int cts_earphone_plugout(struct cts_device *cts_dev)
{
	int ret;
	...
	if (ret) {
		cts_err("Send TP_STD_CMD_SYS_STS_EP_PLUGIN_RW:0 failed %d", ret);
	}
	return ret;
}
#endif
