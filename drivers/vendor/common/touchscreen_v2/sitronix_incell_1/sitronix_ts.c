...
int sitronix_ts_suspend(struct device *dev);
int sitronix_ts_resume(struct device *dev);
...
bool sitronix_ts_get_irq_status(void)
{
	return sitronix_irq_status;
}
...
static irqreturn_t sitronix_ts_irq_handler(int irq, void *data)
{
	...
	for (i = 0; i < ts_data->ts_dev_info.max_touches; i++) {
		if (*p & 0x80) {
			...
			z = *(p + 4);
#ifdef STS_REPORT_BY_ZTE_ALGO
			tpd_touch_press(ts_data->input_dev, x, y, i, 0, 0);
#else
...
static bool sitronix_ts_check_ic_sfrver(void)
{
	...
	if(ret == 0x00 || ret == 0xff){
		stmsg(" GER IC SFR VER failed\n");
		return false;
	}
...
int sitronix_fw_upgrade_handler(void *data)
{
	int ret = 0;
...
int sitronix_touch_bootup_update_check(void)
{
	struct task_struct *fw_boot_th;

	/* return sitronix_touch_check_and_update(upd_data, sizeof(upd_data)); */
	fw_boot_th = kthread_run(sitronix_fw_upgrade_handler, NULL, "sitronix_fw_boot");
	if (fw_boot_th == (struct task_struct *)ERR_PTR) {
		fw_boot_th = NULL;
		WARN_ON(!fw_boot_th);
		sterr("Failed to create fw upgrade thread\n");
		return -EINVAL;
	}
	return 0;
}
...
*/
	...
	sitronix_mt_stop();
#else
	if (!gts->swu_enable || gts->proximity_flag) {
		sitronix_mt_suspend();
	}
...
int sitronix_ts_init(void)
{
	int ret;

	stmsg("%s\n", SITRONIX_TP_DRIVER_VERSION);
	if (get_tp_chip_id() == 0) {
		if ((tpd_cdev->tp_chip_id != TS_CHIP_MAX) && (tpd_cdev->tp_chip_id != TS_CHIP_SITRONIX)) {
			sterr("this tp is not used,return.\n");
			return -EPERM;
		}
	}
	if (tpd_cdev->TP_have_registered) {
		sterr("TP have registered by other TP.\n");
		return -EPERM;
	}
#ifdef SITRONIX_INTERFACE_SPI
