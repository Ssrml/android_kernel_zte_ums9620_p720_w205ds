...
*
* File Name: Focaltech_test_ft8205p.c
*
...
*****************************************************************************/
static int ft8205p_short_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    int i = 0;
    ...
    for (i = 0; i < tdata->node.node_num; i++) {
        ...
        adcdata[i] = (67 * tmp_adc - 5115) / (1023 - tmp_adc);
...
static int ft8205p_open_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    int i = 0;
    bool tmp_result = false;
...
static int ft8205p_cb_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    bool tmp_result = false;
    int byte_num = 0;
...
static int ft8205p_rawdata_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    bool tmp_result = false;
    int *rawdata = NULL;
...
static int ft8205p_lcdnoise_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    int i = 0;
    bool tmp_result = false;
...
static int ft8205p_stylus_check_mode(bool mode)
{
    int ret = 0;
    u8 reg_value = 0;
    u8 write_value = mode ? 0x02 : 0x01;
    int i = 0;
    ...
    do {
        ret = fts_test_write_reg(0x14, write_value);
        if (ret < 0) {
            FTS_TEST_SAVE_ERR("write %d to 0x14 fail\n", mode ? 0x02 : 0x01);
            continue;
        }

        ret = fts_test_read_reg(0x14, &reg_value);
        if (ret < 0) {
            FTS_TEST_SAVE_ERR("read reg 0x14 fail\n");
            continue;
        }

        if (write_value == reg_value) {
            ret = wait_state_update(TEST_RETVAL_00);
            if (ret < 0) {
                FTS_TEST_SAVE_ERR("wait state update fail\n");
                continue;
    ...
    if (i >= 3) {
        FTS_TEST_SAVE_ERR("ft8205p_stylus_check_mode fail\n");
        ret = -EINVAL;
        goto err;
    }
    ret = 0;
...
static int start_test_ft8205p(void)
{
    int ret = 0;
    ...
    ret = ft8205p_stylus_check_mode(0);
    if (ret < 0) {
        FTS_TEST_ERROR("ft8205p_stylus_check_mode fail");
        return false;
    }

    /* short test */
    if (true == test_item->short_test) {
        ret = ft8205p_short_test(tdata, &temp_result);
    ...
    if (true == test_item->open_test) {
        ret = ft8205p_open_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
    ...
    if (true == test_item->cb_test) {
        ret = ft8205p_cb_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
    ...
    if (true == test_item->rawdata_test) {
        ret = ft8205p_rawdata_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
			fts_tptest_result = fts_tptest_result | TEST_BEYOND_MAX_LIMIT
						      | TEST_BEYOND_MIN_LIMIT;
    ...
    if (true == test_item->lcdnoise_test) {
        ret = ft8205p_lcdnoise_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
			fts_tptest_result = fts_tptest_result | TEST_BEYOND_MAX_LIMIT
						      | TEST_BEYOND_MIN_LIMIT;
    ...
    ret = ft8205p_stylus_check_mode(1);
    if (ret < 0) {
        FTS_TEST_ERROR("ft8205p_stylus_check_mode fail");
        return false;
    }
...
struct test_funcs test_func_ft8205p = {
    .ctype = {0x2C},
    .hwtype = IC_HW_INCELL,
    .key_num_total = 0,
    .start_test = start_test_ft8205p,
};
