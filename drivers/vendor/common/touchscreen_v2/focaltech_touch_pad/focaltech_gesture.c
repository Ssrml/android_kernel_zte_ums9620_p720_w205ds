...
static void fts_gesture_report(struct input_dev *input_dev, int gesture_id)
{
    ...
    /* report event key */
	if (gesture == KEY_GESTURE_DOUBLEC) {
		FTS_DEBUG("Gesture Code=%d", gesture);
		if (tpd_cdev->tpd_report_uevent != NULL) {
			FTS_INFO("Double Click Uevent\n");
...
void fts_tp_irq_wake(struct fts_ts_data *ts_data, bool enable)
{
	if (enable && !ts_data->irq_wake_enabled) {
		enable_irq_wake(ts_data->irq);
		ts_data->irq_wake_enabled = true;
		FTS_INFO("enable_irq_wake");
	} else if (!enable && ts_data->irq_wake_enabled) {
