...
#endif
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC) && defined(CONFIG_TOUCHSCREEN_POINT_SIMULAT_UF)
...
#endif
	if (core_data->ztec.tp_report_rate != 1) {
...
static int goodix_drm_register(struct device *dev)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (node != NULL)
			ts_err("%s: node = %s", __func__, node->name);
		...
		if (PTR_ERR(panel) == -ENODEV) {
			ts_err("%s: no device!", __func__);
			retval = -ENODEV;
		} else if (PTR_ERR(panel) == -EPROBE_DEFER) {
			ts_err("%s: device has not been probed yet", __func__);
			retval = -EPROBE_DEFER;
		}
...
int goodix_ts_stage2_init(struct goodix_ts_core *cd)
{
	...
	if (goodix_drm_register(cd->bus->dev) == 0) {
		if (drm_panel_notifier_register(active_panel, &cd->fb_notifier) < 0)
			ts_err("Failed to register drm notifier client\n");
	} else {
		ts_err("Failed to register drm notifier client\n");
	}
#elif defined(CONFIG_FB)
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	tpd_cdev->tp_chip_id = TS_CHIP_GOODIX;
...
int  goodix_ts_core_init(void)
{
	...
	if (get_tp_chip_id() == 0) {
		if ((tpd_cdev->tp_chip_id != TS_CHIP_MAX) && (tpd_cdev->tp_chip_id != TS_CHIP_GOODIX)) {
			ts_err("this tp is not used,return.\n");
			return -EPERM;
		}
	}
	if (tpd_cdev->TP_have_registered) {
		ts_err("TP have registered by other TP.\n");
		return -EPERM;
	}
