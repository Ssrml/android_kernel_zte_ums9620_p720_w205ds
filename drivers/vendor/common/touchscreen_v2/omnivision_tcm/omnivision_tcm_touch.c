...
static void touch_report(void)
{
	...
	if (tcm_hcd->wakeup_gesture_enabled) {
		if (touch_data->gesture_id == GESTURE_DOUBLE_TAP && tcm_hcd->in_suspend) {
			ovt_info(INFO_LOG, "%s:GESTURE_DOUBLE_TAP\n", __func__);
			if (tpd_cdev->tpd_report_uevent != NULL) {
				ovt_info(INFO_LOG, "Double Click Uevent\n");
...
#ifdef OVT_TCM_REPORT_BY_ZTE_ALGO
			if (tpd_cdev->zte_tp_algo) {
				tpd_touch_release(touch_hcd->input_dev, idx);
			} else {
#ifdef TYPE_B_PROTOCOL
			input_mt_slot(touch_hcd->input_dev, idx);
			input_mt_report_slot_state(touch_hcd->input_dev,
...
#ifdef OVT_TCM_REPORT_BY_ZTE_ALGO
			if (tpd_cdev->zte_tp_algo) {
				tpd_touch_press(touch_hcd->input_dev, x, y, idx, 0, 0);
			} else {
#ifdef TYPE_B_PROTOCOL
				input_mt_slot(touch_hcd->input_dev, idx);
				input_mt_report_slot_state(touch_hcd->input_dev,
		...
		const char *buf, size_t count)
{
	...
	tcm_hcd->ovt_proximity_enable = enable;
	ovt_tcm_set_func_face_detect_en_state(enable);
...
int ovt_proximity_input_init(void)
{
	...
	if (touch_hcd->proximity_input_dev == NULL) {
		ovt_info(ERR_LOG, "%s: could not allocate proximity input device\n", __func__);
		return -ENODEV;
	}
