...
/*zte_add*/
struct zte_common_interface_member {
	int panel_max_x;
	int panel_max_y;
	//int ovt_rst_gpio;
	unsigned int rawdata_cols;
	unsigned int rawdata_rows;
	unsigned int fw_ver;
...
struct ovt_tcm_hcd {
...
#if defined(CONFIG_TOUCHSCREEN_DRM_PANEL_NOTIFIER) || defined(CONFIG_FB)
	...
	/*zte_add*/
	struct zte_common_interface_member zte_ctrl;
...
static inline ssize_t ovt_tcm_show_error(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ovt_info(ERR_LOG, "%s:Attribute not readable\n", __func__);
...
static inline ssize_t ovt_tcm_store_error(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	ovt_info(ERR_LOG, "%s:Attribute not writable\n", __func__);
		...
		unsigned int count)
{
	...
	if (count > dest_size || count > src_size) {
		ovt_info(ERR_LOG, "%s:src_size = %d, dest_size = %d, count = %d\n",
		...
		struct ovt_tcm_buffer *buffer, unsigned int size)
{
	...
	if (size > buffer->buf_size) {
		...
		if (!(buffer->buf)) {
			ovt_info(ERR_LOG, "%s: Failed to allocate memory\n", __func__);
			kfree(temp);
			buffer->buf_size = 0;
			return -ENOMEM;
		}
		...
		retval = secure_memcpy(buffer->buf,
				...
				buffer->buf_size);
		if (retval < 0) {
			ovt_info(ERR_LOG, "%s:Failed to copy data\n", __func__);
			kfree(temp);
			kfree(buffer->buf);
			buffer->buf_size = 0;
			return retval;
		...
		struct ovt_tcm_buffer *buffer, unsigned int size)
{
	if (size > buffer->buf_size) {
		...
		if (!(buffer->buf)) {
			ovt_info(ERR_LOG, "%s:Failed to allocate memory\n", __func__);
			ovt_info(ERR_LOG, "%s:Allocation size = %d\n", __func__, size);
			buffer->buf_size = 0;
			buffer->data_length = 0;
			return -ENOMEM;
		}
