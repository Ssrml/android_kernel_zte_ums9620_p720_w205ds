...
#define OVT_TP_DRIVER_VERSION	"v2023-12-22"
...
\
	if ((!tcm_hcd) || (tcm_hcd->ovt_tcm_driver_removing)) { \
		ovt_info(ERR_LOG, "tcm shutdown, do not #c_name\n"); \
		return 0; \
	} \
...
\
		...
		if (retval != 0) { \
			ovt_info(ERR_LOG, "Failed to set #c_name command\n"); \
		} \
	} \
	return retval; \
} \
...
static void ovt_tcm_module_work(struct work_struct *work)
{
	...
	mutex_unlock(&mod_pool.mutex);
	/*zte_add*/
	ominivision_tpd_register_fw_class(tcm_hcd);
...
static void ovt_tcm_dispatch_report(struct ovt_tcm_hcd *tcm_hcd)
{
	...
	ovt_info(DEBUG_LOG, "%s:tcm_hcd->report.id is 0x%x\n", __func__, tcm_hcd->report.id);
		...
		bool config, int dir, int state)
{
	...
	if (config) {
		...
		if (retval < 0) {
			...
			ovt_info(INFO_LOG, "%s:Success to set GPIO label\n", __func__);
		}
		...
		if (retval < 0) {
			...
			ovt_info(INFO_LOG, "%s:Success to request GPIO %d\n", __func__, gpio);
		}
		...
		if (retval < 0) {
			...
			ovt_info(INFO_LOG, "%s:Success to set GPIO %d direction\n", __func__, gpio);
		}
	} else {
		gpio_free(gpio);
	}
...
static int ovt_tcm_config_gpio(struct ovt_tcm_hcd *tcm_hcd)
{
	...
	if (bdata->irq_gpio >= 0) {
		...
		if (retval < 0) {
			...
			ovt_info(INFO_LOG, "%s:Success to configure interrupt GPIO\n", __func__);
		}
	}
	...
	if (bdata->power_gpio >= 0) {
		...
		if (retval < 0) {
			...
			ovt_info(INFO_LOG, "%s:Success to configure power GPIO\n", __func__);
		}
	}
	...
	if (bdata->reset_gpio >= 0) {
		...
		if (retval < 0) {
			...
			ovt_info(INFO_LOG, "%s:Success to configure reset GPIO\n", __func__);
		}
	}

	if (bdata->power_gpio >= 0) {
		gpio_set_value(bdata->power_gpio, bdata->power_on_state);
		ovt_info(INFO_LOG, "%s:gpio_set_value power_gpio(%d) %d\n", __func__,
	...
	if (bdata->reset_gpio >= 0) {
		...
		ovt_info(INFO_LOG, "%s:gpio_set_value reset_gpio(%d) %d\n", __func__,
...
static int ovt_tcm_enable_regulator(struct ovt_tcm_hcd *tcm_hcd, bool en)
{
	...
	if (tcm_hcd->bus_reg) {
		...
		if (retval < 0) {
			...
			ovt_info(INFO_LOG, "%s:Success to enable bus regulator\n", __func__);
		}
	} else {
		ovt_info(INFO_LOG, "%s:Not use bus_reg\n", __func__);
	}
	...
	if (tcm_hcd->pwr_reg) {
		...
		if (retval < 0) {
			...
			ovt_info(INFO_LOG, "%s:Success to enable power regulator\n", __func__);
		}
		msleep(bdata->power_delay_ms);
	} else {
...
static int ovt_tcm_get_regulator(struct ovt_tcm_hcd *tcm_hcd, bool get)
{
	...
	if (bdata->bus_reg_name != NULL && *bdata->bus_reg_name != 0) {
		...
		if (IS_ERR(tcm_hcd->bus_reg)) {
			...
			ovt_info(INFO_LOG, "%s:Success to get bus regulator\n", __func__);
		}
	} else {
		ovt_info(INFO_LOG, "%s:Not use bus_reg\n", __func__);
	}
	...
	if (bdata->pwr_reg_name != NULL && *bdata->pwr_reg_name != 0) {
		...
		if (IS_ERR(tcm_hcd->pwr_reg)) {
			...
			ovt_info(INFO_LOG, "%s:Success to get power regulator\n", __func__);
		}
	} else {
		ovt_info(INFO_LOG, "%s:Not use pwr_reg\n", __func__);
	}
		...
		bool hw, bool update_wd)
{
	...
	if (hw) {
		...
		ovt_info(INFO_LOG, "%s enter HW  reset!\n", __func__);
		gpio_set_value(bdata->reset_gpio, bdata->reset_on_state);
		msleep(bdata->reset_active_ms);
		gpio_set_value(bdata->reset_gpio, !bdata->reset_on_state);
	} else {
...
static void ovt_tcm_helper_work(struct work_struct *work)
{
	...
	case HELP_SEND_REINIT_NOTIFICATION:
		...
		if (retval < 0) {
			...
			ovt_info(INFO_LOG, "%s:reset_gpio=%d\n", __func__, tcm_hcd->hw_if->bdata->reset_gpio);
...
#if defined(CONFIG_PM) || defined(CONFIG_TOUCHSCREEN_DRM_PANEL_NOTIFIER) || defined(CONFIG_FB)
int ovt_tcm_resume(struct device *dev)
{
#if SPEED_UP_RESUME
	...
	if (tcm_hcd->in_hdl_mode) {
		ovt_info(INFO_LOG, "%d:%s:[PATCH] always wait for hdl done when in no flash project\n",
				__LINE__, __func__);
		tcm_hcd->enable_irq(tcm_hcd, true, NULL);
		retval = ovt_tcm_wait_hdl(tcm_hcd);
		if (retval < 0) {
		...
		ovt_info(DEBUG_LOG, "%d:%s:tcm_hcd->wakeup_gesture_enabled=%d\n", __LINE__, __func__,
...
static void speedup_resume(struct work_struct *work)
{
	...
	ovt_info(ERR_LOG,"speed up resume enter\n");
	if (!tcm_hcd->in_suspend  || tcm_hcd->ovt_tcm_driver_removing)
		return;
...
exit:
	tcm_hcd->in_suspend = false;
	ovt_info(ERR_LOG,"speed up resume end\n");
	return;
}
#endif
...
int ovt_tcm_suspend(struct device *dev)
{
	...
	mutex_unlock(&mod_pool.mutex);
	ovt_info(INFO_LOG, "%d:%s:tcm_hcd->wakeup_gesture_enabled=%d\n", __LINE__, __func__,
						tcm_hcd->wakeup_gesture_enabled);
	if (!tcm_hcd->wakeup_gesture_enabled)
		tcm_hcd->enable_irq(tcm_hcd, false, true);
...
static int ovt_tcm_probe(struct platform_device *pdev)
{
	...
	if (!hw_if) {
		...
		ovt_info(INFO_LOG, "%s:Hardware interface found\n", __func__);
	}
	...
	if (!bdata) {
		...
		ovt_info(INFO_LOG, "%s:Board data found\n", __func__);
	}
	...
	if (!tcm_hcd) {
		...
		ovt_info(INFO_LOG, "%s:Success to allocate memory for tcm_hcd\n", __func__);
	}
	...
	tcm_hcd->is_detected = false;
	tcm_hcd->wakeup_gesture_enabled = false; /*zte_modify*/
	tcm_hcd->enter_gesture = false; /*zte_add*/
	...
	ovt_info(INFO_LOG, "%s:tcm_hcd->irq=%d\n", __func__, tcm_hcd->irq);

	/*zte_add*/
	tcm_hcd->zte_ctrl.panel_max_x = bdata->panel_max_x;
	tcm_hcd->zte_ctrl.panel_max_y = bdata->panel_max_y;
	ovt_info(INFO_LOG, "%s:PANEL_MAX_X:%d\n", __func__, tcm_hcd->zte_ctrl.panel_max_x);
	ovt_info(INFO_LOG, "%s:PANEL_MAX_Y:%d\n", __func__, tcm_hcd->zte_ctrl.panel_max_y);
	...
	if (retval < 0) {
		...
		ovt_info(INFO_LOG, "%s:Success to allocate memory for tcm_hcd->in.buf\n", __func__);
	}
	...
	if (retval < 0) {
		...
		ovt_info(INFO_LOG, "%s:Success to get regulators\n", __func__);
	}
	...
	if (retval < 0) {
		...
		ovt_info(INFO_LOG, "%s:Success to enable regulators\n", __func__);
	}
	...
	if (retval < 0) {
		...
		ovt_info(INFO_LOG, "%s:Success to configure GPIO's\n", __func__);
	}
	...
	if (!tcm_hcd->dynamnic_config_sysfs_dir) {
		...
		ovt_info(INFO_LOG, "%s:Success to create dynamic config sysfs directory\n", __func__);
	}
	...
	ovt_info(INFO_LOG, "%s:Success to create dynamic config sysfs file\n", __func__);
#ifdef CONFIG_TOUCHSCREEN_DRM_PANEL_NOTIFIER
	...
	if (retval < 0) {
		...
		ovt_info(INFO_LOG, "%s:Success to register FB notifier client\n", __func__);
	}
#endif
	...
	if (IS_ERR(tcm_hcd->notifier_thread)) {
		...
		ovt_info(INFO_LOG, "%s:Success to create and run tcm_hcd->notifier_thread\n", __func__);
	}
#endif
	...
	if (retval < 0) {
		...
		ovt_info(INFO_LOG, "%s:ovt_tcm_identify success\n", __func__);
	}
	...
	if (retval < 0) {
		...
		ovt_info(INFO_LOG, "%s:Success to initialze touch reporting\n", __func__);
	}
...
int ovt_tcm_module_init(void)
{
	int retval;
	ovt_info(INFO_LOG, "%s enter! driver version:%s\n", __func__, OVT_TP_DRIVER_VERSION);

	if (get_tp_chip_id() == 0) {
		if ((tpd_cdev->tp_chip_id != TS_CHIP_MAX) && (tpd_cdev->tp_chip_id != TS_CHIP_OMNIVISION)) {
			ovt_info(ERR_LOG, "%s:this tp is not used, return\n", __func__);
			return -EPERM;
		}
	}
