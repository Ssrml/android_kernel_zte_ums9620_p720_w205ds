...
#if defined(CONFIG_TOUCHSCREEN_POINT_SIMULAT_UF)
		uf_touch_report(false, -1, -1, finger_count);
#endif
#if defined(CONFIG_TOUCHSCREEN_UFP_MAC) && defined(ZTE_ONE_KEY)
		one_key_report(false, -1, -1, finger_count);
#endif
	}
#else
...
#elif defined(CONFIG_TOUCHSCREEN_DRM_PANEL_NOTIFIER)
...
#endif
#ifdef CONFIG_TOUCHSCREEN_UFP_MAC
		if (ufp_tp_ops.aod_fp_down) {  /*(ts_data->ztec.is_one_key)*/
...
*/
	...
	tpd_cdev->tp_chip_id = TS_CHIP_FOCAL;
...
int  fts_ts_init(void)
{
	...
	ret =  get_tp_chip_id();
	if (ret == 0) {
		if ((tpd_cdev->tp_chip_id != TS_CHIP_MAX) && (tpd_cdev->tp_chip_id != TS_CHIP_FOCAL)) {
			FTS_INFO("this tp is not used,return.\n");
			return -EPERM;
		}
	}
	if (tpd_cdev->TP_have_registered) {
		FTS_INFO("TP have registered by other TP.\n");
		return -EPERM;
	}
