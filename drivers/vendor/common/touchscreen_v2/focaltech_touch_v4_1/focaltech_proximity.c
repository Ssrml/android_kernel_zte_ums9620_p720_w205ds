...
int fts_proximity_init(struct fts_ts_data *ts_data)
{
	...
	{
		FTS_INFO("%s: could not allocate input device\n", __func__);
		return -1;
	}
    FTS_INFO("[FTS_TS]%s: GZL TP_PS init1\n", __func__);
	...
	if (err < 0)
	{
		FTS_INFO("%s: could not register psensor input device\n", __func__);
		goto free_psensor_input_dev;
	}
	/*test PS is OK,and ingnore the first ps value*/
	 FTS_INFO("[FTS_TS]%s: GZL TP_PS init2\n", __func__);
	tpd_enable_ps(ts_data, 1);
	get_ps_mode_data(&ps_value, ts_data);
	if (ts_data->tpd_proximity_flag == 1)
	{
		if (ps_value == 0x01)
		{
			ts_data->tpd_proximity_detect_is_far = 0;  //0-->near ; 1--> far away
		}
		else if (ps_value == 0x00)
		{
			ts_data->tpd_proximity_detect_is_far = 1; //0-->near ; 1--> far away
		}
		ps_value = tpd_get_ps_value(ts_data);
		FTS_INFO("ps_vale: %d\n", ps_value);
		input_report_abs(ts_data->ft6x06_proximity_input_dev, ABS_DISTANCE, ps_value);
		input_sync(ts_data->ft6x06_proximity_input_dev);
	}
	tpd_enable_ps(ts_data, 0);
