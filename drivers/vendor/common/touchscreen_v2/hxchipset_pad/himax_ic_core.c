...
void himax_mcu_touch_information(void)
{
	...
	if (himax_differ_config) {
		if (hx_s_ic_data->rx_num == 0xFFFFFFFF)
			hx_s_ic_data->rx_num = FIX_HX_RX_NUM_CONFIG;

		if (hx_s_ic_data->tx_num == 0xFFFFFFFF)
			hx_s_ic_data->tx_num = FIX_HX_TX_NUM_CONFIG;

		if (hx_s_ic_data->int_is_edge == 0xFF)
			hx_s_ic_data->int_is_edge = FIX_HX_INT_IS_EDGE_CONFIG;
		if (hx_s_ic_data->stylus_func == 0xFF)
			hx_s_ic_data->stylus_func = FIX_HX_STYLUS_FUNC_CONFIG;
