...
extern bool himax_differ_config;
#endif
...
#if defined(HX_PEN_ONLY_SWITCH)
uint8_t g_pen_switch_flag;
EXPORT_SYMBOL(g_pen_switch_flag);
#endif
/*ts_work about start*/
...
#if defined(HX_PEN_ONLY_SWITCH)
#define HX_PROC_PSWITCH_FILE "pen_switch"
struct proc_dir_entry *proc_PSWITCH_file;
#endif
...
{
	uint32_t tmp_addr = 0x10007FE4;
	uint8_t tmp_data[DATA_LEN_4] = {0x00,0x00,0x00,0x00};

	size_t count = 0;
	char *temp_buf = NULL;

	hx_s_core_fp._register_read(tmp_addr, tmp_data, 4);
	I("%s, 0x10007FE4 = : %d %d %d %d\n", __func__, tmp_data[3],tmp_data[2],tmp_data[1],tmp_data[0]);

	if (!HX_PROC_SEND_FLAG) {
	temp_buf = kcalloc(len, sizeof(char), GFP_KERNEL);
	if (temp_buf != NULL) {
		count = snprintf(temp_buf, PAGE_SIZE, "pen switch is :%d\n",
				g_pen_switch_flag);
	   ...
	   size_t len, loff_t *pos)
{
	...
	if (ret < 0 ){
			E("%s: SET PEN ONLY MDOE FAIL.\n", __func__);
			return -EINVAL;
	}else {
		I("%s: pen_switch_flag = %d.\n", __func__, g_pen_switch_flag);
	}
	return len;
}

static const struct proc_ops_name hx_proc_PSWITCH_ops = {
	owner_line
	.proc_op(read)= himax_PSWITCH_read,
	.proc_op(write) = himax_PSWITCH_write,
	};
#endif
...
int himax_common_proc_init(void)
{
...
#endif
#if defined(HX_PEN_ONLY_SWITCH)
	proc_PSWITCH_file = proc_create(HX_PROC_PSWITCH_FILE, 0666,
			hx_touch_proc_dir, &hx_proc_PSWITCH_ops);
	if (proc_PSWITCH_file == NULL) {
	...
	return 0;
#if defined(HX_PEN_ONLY_SWITCH)
    remove_proc_entry(HX_PROC_PSWITCH_FILE, hx_touch_proc_dir);
fail_8:
#endif
#if defined(HX_HEADPHONE)
fail_7:
...
static void himax_wake_event_report(void)
{
	...
	if (KEY_EVENT) {
		...
		if (KEY_EVENT == HX_KEY_DOUBLE_CLICK) {
...
#ifdef HXTP_DIFFER_CONFIG
	if (himax_differ_config) {
		hx_s_touch_data->touch_all_size =
			HX_FULL_STACK_RAWDATA_SIZE_CONFIG;
...
int himax_chip_common_init(void)
{
	...
	if (himax_differ_config) {
		ts->xfer_buff = devm_kzalloc(ts->dev,
			HX_FULL_STACK_RAWDATA_SIZE_CONFIG * sizeof(uint8_t), GFP_KERNEL);
	} else {
		ts->xfer_buff = devm_kzalloc(ts->dev,
			HX_FULL_STACK_RAWDATA_SIZE * sizeof(uint8_t), GFP_KERNEL);
	}
#else
	ts->xfer_buff = devm_kzalloc(ts->dev,
