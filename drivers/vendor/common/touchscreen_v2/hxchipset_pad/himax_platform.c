...
static void himax_ts_isr_func(struct himax_ts_data *ts)
{
    ...
    if ((ts->suspended) && (ts->pm_suspend)) {
        ...
        if (!ret) {
            E("Bus don't resume from pm(deep),timeout,skip irq");
            return;
        }
    }
#endif
...
#if 0 /* defined(HX_CONFIG_DRM) || defined(HX_CONFIG_FB) || defined(HX_CONFIG_PM) */
...
static ssize_t hx_ts_suspend_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return 0;
}
static ssize_t hx_ts_suspend_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((buf[0] == '1') && !hx_s_ts->suspended)
		himax_chip_common_suspend(hx_s_ts);
	else if ((buf[0] == '0') && hx_s_ts->suspended)
		himax_chip_common_resume(hx_s_ts);
	return count;
}
static DEVICE_ATTR_RW(hx_ts_suspend);
static struct attribute *himax_dev_suspend_atts[] = {
	&dev_attr_hx_ts_suspend.attr,
	NULL
};
...
#if defined(HX_CONFIG_PM)
static int hxtp_pm_suspend(struct device *dev)
{
    struct himax_ts_data *ts = hx_s_ts;
...
static int hxtp_pm_resume(struct device *dev)
{
    struct himax_ts_data *ts = hx_s_ts;
...
static const struct dev_pm_ops himax_common_pm_ops = {
    .suspend = hxtp_pm_suspend,
    .resume = hxtp_pm_resume,
};
...
int himax_common_init(void)
{
	...
	I("Himax common touch panel driver init 20231218\n");
	...
	ret =  get_tp_chip_id();
	if (ret == 0) {
		if ((tpd_cdev->tp_chip_id != TS_CHIP_MAX) && (tpd_cdev->tp_chip_id != TS_CHIP_HIMAX)) {
