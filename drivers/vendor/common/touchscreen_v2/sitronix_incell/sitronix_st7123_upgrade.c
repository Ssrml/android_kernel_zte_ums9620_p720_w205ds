	...
	*fwOff = 0;
	...
	if(data[0x84] == 0){
		...
		*fwInfoOff |= (data[0x98]);
		...
		fwCrcOff |= (data[0x87]);
		*fwSize = fwCrcOff+1;
		if(gts->ts_dev_info.chip_ver < 0x47){
			sterr("ChipVer(0x%02X) does NOT support 72K firmware!\n", gts->ts_dev_info.chip_ver);
			return -EINVAL;
		}
	}
	else{
...
int sitronix_do_upgrade_flash_icp_all(void)
{
	...
	if ( !gts->flash_powerful_upgrade) {
		...
		cfgDramOff = (dump_buf[cfgFlashOff + SITRONIX_FLASH_CFG_TAB_VER_OFF] << 8) + dump_buf[cfgFlashOff + SITRONIX_FLASH_CFG_TAB_VER_OFF + 1];
	...
	//find fwInfoOff on Flash
	if (st_icp_flash_read(fw_check, 0, ST_ICP_READ_BLOCK_SIZE) < 0) {
		sterr("read flash fail @0x0 , cancel upgrade\n");
		rt = -1;
		goto ST_ICP_HW_RESET;
	}

	flashFwInfoOff = fw_check[0x93] << 8 | fw_check[0x94];
	flashFwInfoOff += 4;

 	if (st_icp_flash_read(fw_check, flashFwInfoOff, fwInfoLen) < 0) {
		sterr("read flash fail @0x%X , cancel upgrade\n", fwInfoOff);
		rt = -1;
		goto ST_ICP_HW_RESET;
	}
	...
	while (isSuccess == -1 && retry++  < ST_RAM_MODE_RETRY_MAX) {
		...
		if(gts->ts_dev_info.chip_ver >= 0x47){
			if(buf[0x84] == 0){	//large firmware
				ret = sitronix_spi_pram_rw(false, ST_HDL_PRAM_ADDR_PART2, buf+ST_FW_LEN, NULL, ST_FW_LEN_PART2);
				if (ret < 0) {
...
void sitronix_replace_dump_buf(unsigned char *id)
{
	...
	if(id[0] == dump_id_1[0] && id[1] == dump_id_1[1] && id[2] == dump_id_1[2]) {
		...
		strcpy(fw_path, ST_REQUESTFW_PATH_ID1);
#else
		//do replace here
