...
#include <vendor/common/zte_lcd_notifier.h>
...
struct misc_dev_data {
	...
	struct zte_power_supply	*zte_interface_psy;
...
extern int vote(struct votable *votable, const char *client_str, bool enabled, int val);
...
int zte_screen_on_get(char *val, const void *arg)
{
	...
	return snprintf(val, PAGE_SIZE, "%u", misc_hal_data.screen_on);
}

static int zte_screen_on_set(const char *val, const void *arg)
{
	int screen_on_now = 0, ret = 0;

	if (sscanf(val, "%d", &screen_on_now) != 1) {
		pr_err("%s: sscanf screen_on failed\n", __func__);
		return -EINVAL;
	}

	misc_hal_data.screen_on = screen_on_now;

	ret = sqc_power_supply_changed();
	if (ret) {
		pr_err("%s: sqc_power_supply_changed err, ret = %d\n", __func__, ret);
	}

	pr_info("zte_screen_on_set screen_on %d\n", misc_hal_data.screen_on);
...
int zte_sqc_set_prop_by_name(const char *name, enum zte_power_supply_property psp, int data)
{
...
EXPORT_SYMBOL_GPL(zte_sqc_set_prop_by_name);
#endif
...
EXPORT_SYMBOL_GPL(zte_sqc_get_property);
...
EXPORT_SYMBOL_GPL(zte_sqc_set_property);


static int sqc_set_raw_soc_statue(bool flag)
{
	if ((flag)&&(misc_hal_data.raw_soc_enabled != true)) {
		zte_sqc_set_prop_by_name("zte_battery", POWER_SUPPLY_PROP_CAPACITY_RAW, true);
	...
	if ((!flag)&&(misc_hal_data.raw_soc_enabled != false)) {
		zte_sqc_set_prop_by_name("zte_battery", POWER_SUPPLY_PROP_CAPACITY_RAW, false);
...
static int sqc_set_oem_type(struct misc_dev_data *pdata)
{
	...
	if ((pdata->chg_type == SQC_SLEEP_MODE_TYPE)
			|| (pdata->chg_type == SQC_DOZE_MODE_TYPE)) {
	...
	switch (pdata->chg_type) {
	case SQC_PD3D0_APDO_TYPE:
		pr_info("charing type: CHARGER_TYPE_SQC, ICL %d\n", misc_hal_data.deamon_pps_icl);
		if (misc_hal_data.deamon_pps_icl >= 5000) {
			charge_type_oem = CHARGER_TYPE_SQC;
		} else {
	...
	case SQC_QC3D0_PLUS_45W_TYPE:
		...
		break;
	case SQC_SFCP_1P0_TYPE:
		pr_info("charing type: SQC_SFCP_1P0_TYPE\n");
		charge_type_oem = CHARGER_TYPE_SFCP;
		break;
	...
	misc_hal_data.fast_chg = ((charge_type_oem == CHARGER_TYPE_FAST_CHARGER)
								|| (charge_type_oem == CHARGER_TYPE_SQC)) ? true : false;
	pr_info("%s charge_type_oem %d, get_boot_mode %d screen_on:%d fast_chg:%d\n",
		__func__, charge_type_oem, sqc_get_boot_mode(), misc_hal_data.screen_on, misc_hal_data.fast_chg);

	if (sqc_get_boot_mode() == SQC_BOOT_NORMAL) {
	...
	unsigned long event, void *data)
{
	...
	case LCD_POWER_ON:
		misc_hal_data.screen_on = true;
		pr_info("%s: lcd power on\n", __func__);
		break;
	case LCD_RESET:
		pr_info("%s: lcd reset\n", __func__);
		break;
	case LCD_CMD_ON:
		pr_info("%s: lcd cmd on\n", __func__);
		break;
	case LCD_CMD_OFF:
		pr_info("%s: lcd cmd off\n", __func__);
		break;
	case LCD_POWER_OFF:
		misc_hal_data.screen_on = false;
		pr_info("%s: lcd power off\n", __func__);
		break;
	default:
		pr_info("%s: lcd state unknown event:%d\n", __func__, event);
		break;
	}
			   ...
			   size_t count, loff_t *f_pos)
{
	...
	case SQC_IFS_PMIC_ICL:
		...
		if (misc_hal_data.thermal_ibus_limit >= 0)
			pr_info("read thermal_ibus_limit: %d, %s\n",
					misc_hal_data.thermal_ibus_limit, get_effective_client(misc_hal_data.usb_icl_votable));
		break;
	case SQC_IFS_PMIC_FCV:
		temp_val[0] = misc_hal_data.thermal_vbat_limit;
		read_cnt = sizeof(temp_val[0]);
		if (misc_hal_data.thermal_vbat_limit >= 0)
			pr_info("read thermal_vbat_limit: %d, %s\n",
					misc_hal_data.thermal_vbat_limit, get_effective_client(misc_hal_data.fcv_votable));
		break;
	case SQC_IFS_PMIC_FCC:
		temp_val[0] = misc_hal_data.thermal_ibat_limit;
		read_cnt = sizeof(temp_val[0]);
		if (misc_hal_data.thermal_ibat_limit >= 0)
			pr_info("read thermal_ibat_limit: %d, %s\n",
					misc_hal_data.thermal_ibat_limit, get_effective_client(misc_hal_data.fcc_votable));
		break;
	case SQC_IFS_PMIC_TOPOFF:
		temp_val[0] = misc_hal_data.thermal_topoff_limit;
		read_cnt = sizeof(temp_val[0]);
		if (misc_hal_data.thermal_topoff_limit >= 0)
			pr_info("read thermal_topoff_limit: %d, %s\n",
					misc_hal_data.thermal_topoff_limit, get_effective_client(misc_hal_data.topoff_votable));
		break;
	case SQC_IFS_PMIC_RECHG_SOC:
		temp_val[0] = misc_hal_data.thermal_rechg_soc;
		read_cnt = sizeof(temp_val[0]);
		if (misc_hal_data.thermal_rechg_soc >= 0)
			pr_info("read thermal_rechg_soc: %d, %s\n",
					misc_hal_data.thermal_rechg_soc, get_effective_client(misc_hal_data.recharge_soc_votable));
		break;
	case SQC_IFS_PMIC_RECHG_VOLT:
		temp_val[0] = misc_hal_data.thermal_rechg_volt;
		read_cnt = sizeof(temp_val[0]);
		if (misc_hal_data.thermal_rechg_volt >= 0)
			pr_info("read thermal_rechg_volt: %d, %s\n",
					misc_hal_data.thermal_rechg_volt, get_effective_client(misc_hal_data.recharge_voltage_votable));
		break;
	default:
		retval = -ENOMEM;
			    ...
			    size_t count, loff_t *f_pos)
{
	...
	case SQC_IFS_PTL_COM_STATUS:
		if (misc_hal_data.chg_status != handle_buf[0]) {
			...
			sqc_set_prop_by_name("battery", POWER_SUPPLY_PROP_STATUS, misc_hal_data.chg_status);
			need_update = true;
		}
		break;
	case SQC_IFS_PTL_COM_CHG_TYPE:
		...
		break;
	case SQC_IFS_PMIC_ICL:
		if (misc_hal_data.deamon_pps_icl != handle_buf[0]) {
			misc_hal_data.deamon_pps_icl = handle_buf[0];
			pr_info("write deamon_pps_icl: %d\n", misc_hal_data.deamon_pps_icl);
			sqc_set_oem_type(&misc_hal_data);
		}
		break;
	case SQC_IFS_PMIC_FCC:
		if (misc_hal_data.deamon_fcc != handle_buf[0]) {
			misc_hal_data.deamon_fcc = handle_buf[0];
			pr_info("write deamon_fcc: %d\n", misc_hal_data.deamon_fcc);
			sqc_set_prop_by_name("battery",
				POWER_SUPPLY_PROP_CURRENT_MAX, misc_hal_data.deamon_fcc);
		}
		break;
	case SQC_IFS_PMIC_TOPOFF:
		if (misc_hal_data.deamon_topoff != handle_buf[0]) {
			misc_hal_data.deamon_topoff = handle_buf[0];
			pr_info("write deamon_topoff: %d\n", misc_hal_data.deamon_topoff);
			sqc_set_prop_by_name("battery",
				POWER_SUPPLY_PROP_CHARGE_TERM_CURRENT, misc_hal_data.deamon_topoff);
		}
		break;
	case SQC_IFS_PTL_COM_WAKELOCK:
		if (handle_buf[0]) {
			pm_wakeup_event(misc_hal_data.dev, 16000);
		} else {
			pm_relax(misc_hal_data.dev);
		}
		break;
	default:
		retval = -ENOMEM;
...
static int zte_interface_psy_get_property(struct zte_power_supply *psy,
				enum zte_power_supply_property psp,
				union power_supply_propval *pval)
{
	struct misc_dev_data *pdata = zte_power_supply_get_drvdata(psy);
	int rc = 0;
...
static int zte_interface_psy_set_property(struct zte_power_supply *psy,
				enum zte_power_supply_property psp,
				const union power_supply_propval *pval)
{
	struct misc_dev_data *pdata = zte_power_supply_get_drvdata(psy);
...
static int zte_interface_property_is_writeable(struct zte_power_supply *psy,
					enum zte_power_supply_property psp)
{
	switch (psp) {
	case POWER_SUPPLY_PROP_INPUT_SUSPEND:
...
static void zte_interface_external_power_changed(struct zte_power_supply *psy)
{
	pr_info("zte_power supply changed\n");
}
...
static int charger_manager_chg_type_show(struct seq_file *m, void *v)
{
	int chg_val = 0;

	switch (charge_type_oem) {
	case CHARGER_TYPE_SQC:
...
static int charger_manager_chg_type_open(struct inode *inode, struct file *file)
{
	return single_open(file, charger_manager_chg_type_show, NULL);
}


static const struct file_operations charger_manager_chg_type_node = {
	.owner = THIS_MODULE,
	.open = charger_manager_chg_type_open,
	.read = seq_read,
	.llseek	= seq_lseek,
};
...
static enum zte_power_supply_property zte_interface_psy_props[] = {
...
static const struct zte_power_supply_desc zte_interface_psy_desc = {
	.name = "zte_interface",
	.type = POWER_SUPPLY_TYPE_UNKNOWN,
	.properties = zte_interface_psy_props,
	.num_properties = ARRAY_SIZE(zte_interface_psy_props),
	.get_property = zte_interface_psy_get_property,
	.set_property = zte_interface_psy_set_property,
	.external_power_changed = zte_interface_external_power_changed,
	.property_is_writeable = zte_interface_property_is_writeable,
};
...
static int sqc_misc_init(void)
{
	...
	pr_info("sqc_misc_fops init begin!!!\n ");

	memset(&misc_hal_data, 0, sizeof(misc_hal_data));

	atomic_set(&misc_hal_data.init_finished, 0);

	/* Register the power supply */
	memset(&interface_psy_cfg, 0, sizeof(interface_psy_cfg));
	...
	misc_hal_data.dev = device_create(misc_hal_data.device_class, NULL,
				   sqc_misc_devno, NULL, SQC_MISC_DEVNAME);
	if (IS_ERR_OR_NULL(misc_hal_data.dev)) {
		pr_err("%s: Failed to create sqc_misc char device\n",	__func__);
		goto err_device_region;
	}

	misc_hal_data.interface_psy = power_supply_register(misc_hal_data.dev, &interface_psy_desc,
					&interface_psy_cfg);
	...
	misc_hal_data.zte_interface_psy = zte_power_supply_register(misc_hal_data.dev, &zte_interface_psy_desc,
					&interface_psy_cfg);
	if (IS_ERR(misc_hal_data.zte_interface_psy)) {
		rc = PTR_ERR(misc_hal_data.zte_interface_psy);
		pr_err("failed to register zte_bcl_psy rc = %ld\n",
				PTR_ERR(misc_hal_data.zte_interface_psy));
		goto register_power_supply_failed;
	}
	...
	zte_misc_register_callback(&zte_screen_on_node, &misc_hal_data);

#if 0
	misc_hal_data.fb_notifier.notifier_call = sqc_misc_fb_notifier_callback;
	ret = lcd_notifier_register_client(&misc_hal_data.fb_notifier);
	if (ret) {
	...
	device_init_wakeup(misc_hal_data.dev, true);

	proc_create_data("driver/chg_type", 0664, NULL,
			&charger_manager_chg_type_node, NULL);
...
#if 0
	lcd_notifier_unregister_client(&misc_hal_data.fb_notifier);
#endif

	device_init_wakeup(misc_hal_data.dev, false);
