...
#define SQC_PDO_MAX_OBJECTS 7

#define SQC_PMIC_TYPE_BUCK 		BIT(0)
#define SQC_PMIC_TYPE_BUCKBOOST	BIT(1)
#define SQC_PMIC_TYPE_BYPASS		BIT(2)
...
enum sqc_pd_type {
	...
	SQC_DOZE_MODE_TYPE,
	SQC_SFCP_1P0_TYPE,
	SQC_SFCP_1P0_5V_TYPE,
	SQC_SFCP_1P0_9V_TYPE,
	SQC_SFCP_1P0_12V_TYPE,
};
...
enum sqc_pmic_pos {
	...
	SQC_CHARGER_PARALLEL1,
	SQC_CHARGER_PARALLEL2,
};
...
enum sqc_notify_dev_type {
	...
	SQC_NOTIFY_USBPD,
	SQC_NOTIFY_USB,
	SQC_NOTIFY_SCREEN_ON,
};

enum sqc_chg_status_type {
	SQC_CHG_STATUS_AICL = 0,
};
...
enum sqc_notify_msg {
	...
	SQC_ERR_VBAT_OVP_ALM = 14,
	SQC_ERR_IBAT_OCP_ALM = 15,
	SQC_ERR_IBAT_UCP_ALM = 16,
	SQC_ERR_VBUS_OVP_ALM = 17,
	SQC_ERR_IBUS_OCP_ALM = 18,
	SQC_ERR_ENGINE_OTP_ALM = 19,/*charger otp*/
	SQC_ERR_TSBAT_TSBUS_OTP_ALM = 20,

	SQC_ERR_VDELTA_RATIO_ALM = 21,

	SQC_NOTIFY_USB_STATUS_CHANGED = 22,

	SQC_NOTIFY_FAST_CHG_STATUS = 23,

};

enum sqc_cable_type {
	SQC_CABLE_TYPE_NONE = 0,
	SQC_CABLE_TYPE_A2C,
	SQC_CABLE_TYPE_C2C,
	SQC_CABLE_TYPE_MAX,
};
...
enum sqc_pdos_type {
	SQC_PDOS_TYPE_FIXED = 0,
	SQC_PDOS_TYPE_APDO = 1,
};

struct sqc_pd_adapter_cap {
	uint8_t type[SQC_PDO_MAX_OBJECTS];
	int max_mv[SQC_PDO_MAX_OBJECTS];
	int min_mv[SQC_PDO_MAX_OBJECTS];
	int ma[SQC_PDO_MAX_OBJECTS];
	int pwr_mw_limit[SQC_PDO_MAX_OBJECTS];
...
struct sqc_pd_proto_ops {
	...
	int (*get_cable_type)(int *cable_type);
	int (*set_apdo_cap)(int type, int mV, int mA);
	int (*get_apdo_cap)(struct sqc_pd_adapter_cap *pdos);
};
...
struct sqc_pmic_chg_ops {
	...
	int (*get_chg_type)(void *arg, unsigned int *chg_type);
